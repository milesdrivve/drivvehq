...
const ProductImages = (props) => {
    ...
    const beforeUpload = (file) => {
        ...
        for (let f of file) {
            if (!allowedFileType.includes(f.type)) {
                valid = 'Please upload correct file type!'
            }

            /*if (f.size >= maxFileSize) {
                valid = 'Upload image cannot more than 1MB!'
            }*/
        }
    ...
    return (
        <AdaptableCard className="mb-4">
            <h5>Documents</h5>
            <p className="mb-6">Add or change documents for this customer</p>
            <FormItem>
                <Field name="imgList">
                    {({ field, form }) => {
                        if (values.imgList.length > 0) {
                            return (
                                <div className="grid grid-cols-12 sm:grid-cols-4 lg:grid-cols-12 xl:grid-cols-12 gap-4">
                                    <ImageList
