...
export default function CarsUpdateForm(props) {
  ...
  const resetStateValues = () => {
    ...
    setTransmission(cleanValues.transmission);
    setYear(cleanValues.year);
    setPrice(cleanValues.price);
    setDeposit(cleanValues.deposit);
    setErrors({});
  };
  const [carsRecord, setCarsRecord] = React.useState(cars);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Cars, idProp) : cars;
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Cars.copyOf(carsRecord, (updated) => {
